...
module.exports = function (app) {
  ...
  app.put('/posts/:id', function (request, response) {
    return controllers.post.findById(request.user, request.params.id, function (error, post) {
      if (error) return response.status(500).json(error);
      if (post.user.id != request.user.id) return response.status(401).end();
      return controllers.post.update(post, request.body, function (error, status) {
        if (error) return response.status(500).json(error);
        return response.json(post);
      });
    });
  ...
  app.delete('/posts/:id', function (request, response) {
    return controllers.post.findById(request.user, request.params.id, function (error, post) {
      if (error) return response.status(500).json(error);
      if (post.user.id != request.user.id) return response.status(401).end();
      return controllers.post.remove(post, function (error, status) {
        if (error) return response.status(500).json(error);
  ...
  app.post('/posts/:id/threads', middleware.auth.verify, function (request, response) {
    return controllers.post.findById(request.user, request.params.id, function (error, post) {
      if (error) return response.status(500).json(error);
