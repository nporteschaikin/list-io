...
exports.buildConditions = function (request, response, next) {
  ...
  if (params.start) {
    var startCondition = conditions._id = {}
    var startKey = sortOrder == 'desc' ? '$lt' : '$gt';
    startCondition[startKey] = params.start;
  }
...
/*
 * @function Set paging params.
 */

exports.setPaging = function (request, response, next) {
  ...
  return Models.Picture
    .count(request.conditions, function (error, count) {
      ...
      paging.count = count;
      paging.sort = request.sortConditions;
      paging.limit = request.limitCondition;
      return next();
...
/*
 * @function Like a picture.
 */

exports.like = function (request, response, next) {
  if (!response.picture) return next();
  var picture = response.picture;
  ...
  picture.like(request.sessionUser, function (error) {
    if (error) return next(error);
    return next();
  });
...
/*
 * @function Unlike a picture.
 */

exports.unlike = function (request, response, next) {
  if (!response.picture) return next();
  var picture = response.picture;
  ...
  picture.unlike(request.sessionUser, function (error) {
    if (error) return next(error);
    return next();
  });
